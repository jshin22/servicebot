...
from ai_mbti_analysis import mbti_classify
from recommender import recommender_ai
...
def result():
    if request.method == 'POST':
        ...
        # anaysis mbit
        cv_letter = result
        result = mbti_classify(result)
        # 이 부분에서 시간이 걸림
        df_result__ =result['Type']
        df_result__ = df_result__.values.tolist()
        txt__ = ''.join(df_result__)
        # recommend csr activity
        result = recommender_ai(txt__)
        return render_template("result.html", cv_letter = cv_letter, result = result)
